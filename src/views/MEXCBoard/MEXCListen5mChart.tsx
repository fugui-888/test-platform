...
type Props = {
  candles5: ListenCandle5m[];
  height?: number;
  zAbsThreshold?: number;
};

const MEXCListen5mChart: React.FC<Props> = ({
  candles5,
  height = 550,
  zAbsThreshold = 2.3,
}) => {
  ...
  }, [candles5.length, height]);

  useEffect(() => {
    const chart = chartRef.current;
    if (!chart || candles5.length === 0) return;
    chart.applyNewData(
      candles5.map((c) => ({
        timestamp: c.ts * 1000,
        open: c.open,
  ...
  const zFilterOverlays = useMemo(() => {
    ...
    return out;
  }, [candles5, maStd, overlayTick, zAbsThreshold]);

  const latestIdx = candles5.length - 1;
  const bar = latestIdx >= 0 ? candles5[latestIdx] : null;
...
export default MEXCListen5mChart;
